...
À la racine du projet, tout (MySQL + back + front) peut être lancé avec Docker Compose :

```bash
docker compose up -d --build
```

- **Front** : http://localhost:4300 (proxy `/api` → back)
- **Back** : http://localhost:8080
- **MySQL** : port 3306 (interne au réseau Docker)

Arrêt : `docker compose down`. Données MySQL : volume `mysql_data`.
...
- Justification des choix techniques : `docs/justification-choix-techniques.md`
- Contrat API : `docs/api-contract.md`
- Conformité (privacy, cookies) : `docs/privacy.md`
- FAQ utilisateur : `docs/faq-utilisateur.md`
- Captures de couverture : `docs/coverage-back.png`, `docs/coverage-front.png`, `docs/coverage-cypress.png`
- Captures d’écran de l’UI : `docs/UI/`